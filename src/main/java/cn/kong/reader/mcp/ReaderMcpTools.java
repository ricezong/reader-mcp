...
public class ReaderMcpTools {
    ...
    /** 列出书源（可选按类型筛选） */
    @Tool(name = "reader_list_sources", description = "列出当前所有可用的书源，可按类型筛选。返回书源简称（source）、名称、类型、类型描述。后续操作通过 source 简称指定书源")
    public List<SourceInfo> listSources(
        ...
        try {
            ...
            log.error("MCP reader_list_sources 调用失败: type={}", type, e);
            throw new RuntimeException("获取书源列表失败，请稍后重试");
        }
    }

    /** 按关键词搜索小说（聚合所有小说源或指定单源，支持分页） */
    @Tool(name = "reader_search_novel", description = "按关键词搜索小说。默认聚合所有小说源并行搜索；若传入 source 简称则只搜索该源。支持分页，不传 page 默认第 1 页。返回结果包含 source 字段供后续操作使用")
    public List<SearchResult> search(
        ...
        try {
            ...
            log.error("MCP reader_search_novel 调用失败: keyword={}, source={}, page={}", keyword, source, page, e);
            throw new RuntimeException("搜索失败，请稍后重试或更换书源");
        }
    }

    /** 按关键词搜索漫画（聚合所有漫画源或指定单源，支持分页） */
    @Tool(name = "reader_search_comic", description = "按关键词搜索漫画。默认聚合所有漫画源并行搜索；若传入 source 简称则只搜索该源。支持分页，不传 page 默认第 1 页。返回结果包含 source 字段供后续操作使用")
    public List<SearchResult> searchComic(
        ...
        try {
            ...
            log.error("MCP reader_search_comic 调用失败: keyword={}, source={}, page={}", keyword, source, page, e);
            throw new RuntimeException("搜索漫画失败，请稍后重试或更换书源");
        }
    }

    /** 按作者搜索小说（聚合所有小说源） */
    @Tool(name = "reader_search_novel_by_author", description = "按作者名查询其全部小说作品，聚合所有小说源搜索，引擎自动过滤匹配作者的结果，返回结果包含 source 字段供后续操作使用")
    public List<SearchResult> searchByAuthor(
        ...
        try {
            ...
            log.error("MCP reader_search_novel_by_author 调用失败: author={}", author, e);
            throw new RuntimeException("按作者搜索失败，请稍后重试或更换书源");
        }
    }

    /** 按作者搜索漫画（聚合所有漫画源） */
    @Tool(name = "reader_search_comic_by_author", description = "按作者名查询其全部漫画作品，聚合所有漫画源搜索，引擎自动过滤匹配作者的结果，返回结果包含 source 字段供后续操作使用")
    public List<SearchResult> searchComicByAuthor(
        ...
        try {
            ...
            log.error("MCP reader_search_comic_by_author 调用失败: author={}", author, e);
            throw new RuntimeException("按作者搜索漫画失败，请稍后重试或更换书源");
        }
    }

    /** 获取书籍详情 */
    @Tool(name = "reader_book_info", description = "获取书籍详情（书名、作者、简介、封面URL、字数、最新章节标题、来源简称等）。需要先通过搜索接口获取 source 和 bookUrl。")
    public BookDetail bookInfo(
        ...
        try {
            ...
            log.error("MCP reader_book_info 调用失败: source={}, bookUrl={}", source, bookUrl, e);
    ...
    public List<ChapterInfo> chapters(
        ...
        try {
            ...
            log.error("MCP reader_chapters 调用失败: source={}, bookUrl={}", source, bookUrl, e);
            throw new RuntimeException("获取章节目录失败，请稍后重试");
        }
    }

    /** 获取单章正文（章节序号从 1 开始，内部转换为引擎的 0-based） */
    @Tool(name = "reader_content", description = "获取指定章节的正文内容（章节序号从 1 开始）。小说返回纯文本，漫画返回包含 <img> 标签的 HTML。")
    public String content(
        ...
        try {
            ...
            log.error("MCP reader_content 调用失败: source={}, bookUrl={}, chapterIndex={}", source, bookUrl, chapterIndex, e);
    ...
    public DownloadResult download(
        ...
        try {
            ...
            log.error("MCP reader_download 调用失败: source={}, bookUrl={}, start={}, end={}", source, bookUrl, start, end, e);
