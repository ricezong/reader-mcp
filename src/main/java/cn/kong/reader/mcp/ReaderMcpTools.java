...
/**
 * MCP 工具定义：将搜索、详情、目录、正文、下载能力暴露为 8 个 MCP 工具。
...
public class ReaderMcpTools {
    ...
    /** 按关键词搜索小说（聚合所有小说源或指定单源） */
    @Tool(name = "reader_search", description = "按关键词搜索小说。默认聚合所有小说源并行搜索；若传入 source 简称则只搜索该源。返回结果包含 source 字段供后续操作使用")
    public List<SearchResult> search(
            @ToolParam(description = "搜索关键词") String keyword,
            @ToolParam(description = "书源简称（可选，如 80、dubu）。传入则只搜索该源，不传则聚合所有小说源") String source) {
        try {
            if (source != null && !source.isBlank()) {
                return readerApi.searchBySource(keyword, source, 1);
            }
            return readerApi.searchNovel(keyword);
        } catch (IllegalArgumentException e) {
            throw e;
        } catch (Exception e) {
            log.error("MCP reader_search 调用失败: keyword={}, source={}", keyword, source, e);
            throw new RuntimeException("搜索失败，请稍后重试或更换书源");
        }
    }

    /** 按关键词搜索漫画（聚合所有漫画源或指定单源） */
    @Tool(name = "reader_search_comic", description = "按关键词搜索漫画。默认聚合所有漫画源并行搜索；若传入 source 简称则只搜索该源。返回结果包含 source 字段供后续操作使用")
    public List<SearchResult> searchComic(
        ...
        try {
            ...
            return readerApi.searchComic(keyword);
        } catch (IllegalArgumentException e) {
            throw e;
        } catch (Exception e) {
            log.error("MCP reader_search_comic 调用失败: keyword={}, source={}", keyword, source, e);
            throw new RuntimeException("搜索漫画失败，请稍后重试或更换书源");
        }
    }

    /** 按作者搜索小说（聚合所有小说源） */
    @Tool(name = "reader_search_by_author", description = "按作者名查询其全部小说作品，聚合所有小说源搜索，引擎自动过滤匹配作者的结果，返回结果包含 source 字段供后续操作使用")
    public List<SearchResult> searchByAuthor(
        ...
        try {
            ...
            log.error("MCP reader_search_by_author 调用失败: author={}", author, e);
            throw new RuntimeException("按作者搜索失败，请稍后重试或更换书源");
        }
    }

    /** 获取书籍详情 */
    @Tool(name = "reader_book_info", description = "获取书籍详情（作者、简介、封面、字数、最新章节等）")
    ...
    @Tool(name = "reader_content", description = "获取指定章节的正文内容（章节序号从 1 开始），返回正文文本")
